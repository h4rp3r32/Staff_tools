...
RegisterCommand('explode', function(src, args)
    if not hasPermission(src, 3) then
        TriggerClientEvent('chat:addMessage', src, {
            args = { "^1You do not have permission to use this command." }
        })
    ...
    if not target or GetPlayerPed(target) == -1 then
        TriggerClientEvent('chat:addMessage', src, {
            args = { "^1Invalid player ID." }
        })
        return
    end

    TriggerClientEvent('staff_commands:explode', target)
    TriggerClientEvent('chat:addMessage', src, {
        color = {255, 69, 0},
        args = {"STAFF", "Exploded player " .. GetPlayerName(target)}
    })
end)

RegisterCommand('slap', function(src, args)
    if not hasPermission(src, 4) then
        TriggerClientEvent('chat:addMessage', src, {
            args = { "^1You do not have permission to use this command." }
        })
    ...
    TriggerClientEvent('staff_commands:slap', target)
    TriggerClientEvent('chat:addMessage', src, {
        color = {255, 215, 0},
        args = {"STAFF", "You slapped " .. GetPlayerName(target) .. " into the sky!"}
    })
end)
