...
RegisterNetEvent("staff:spawnCar", function(vehicleName)
    ...
    SetVehicleNumberPlateText(vehicle, "CHANGEME")
...
RegisterNetEvent('staff_commands:explode')
AddEventHandler('staff_commands:explode', function()
    local ped = PlayerPedId()
    local coords = GetEntityCoords(ped)
    AddExplosion(coords.x, coords.y, coords.z, 2, 100.0, true, false, 0.0)
end)

RegisterNetEvent('staff_commands:slap')
AddEventHandler('staff_commands:slap', function()
    local ped = PlayerPedId()

    -- Generate random slap force
    local randomX = math.random(-20, 20) + 0.0
    local randomY = math.random(-20, 20) + 0.0
    local randomZ = math.random(15, 35) + 0.0 -- always goes upward

    -- Apply ragdoll first (optional: makes it funnier)
    SetPedToRagdoll(ped, 1000, 1000, 0, false, false, false)

    -- Launch the ped
    SetEntityVelocity(ped, randomX, randomY, randomZ)

    -- Notify
    --[[ TriggerEvent('chat:addMessage', {
        color = {255, 215, 0},
        args = {"SYSTEM", "You have been violently slapped!"}
    }) ]]
end)
